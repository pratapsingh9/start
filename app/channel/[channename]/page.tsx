  ...
  return (
    <div className="bg-black flex-grow text-white overflow-hidden h-screen w-8/12 flex flex-col border-r border-gray-300">
      <header className="h-16 flex items-center justify-between border-b border-gray-800 px-6">
        <div
          className="h-14 w-14 rounded-full flex items-center justify-center cursor-pointer hover:bg-gray-800"
      ...
      <main className="flex-grow flex flex-col overflow-hidden">
        <ScrollArea className="flex-grow p-4 px-12 overflow-y-auto">
          {messages.map((message) => (
            <div
              key={message.id}
              className="flex items-start my-4 space-x-3 mb-4"
            >
              <Avatar>
...
export default function App({params}:paramProps) {
  return (
    <div className="bg-black h-screen flex">
      <SideBar />
      <MainGroupScreen name={params.channename} />
      <RightBar />
    </div>
  );
}
